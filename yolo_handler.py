...
try:
    ...
    print("Not able to run YOLO on the Intel NCS2 TPU! The OpenVINO SDK should be installed if you intend to run YOLO on the TPU")
try:
    ie = IECore()
except NameError:
    print("Please install/source OpenVino environment to use the NCS2 YOLO Handler.")
...
class YoloHandler():
    ...
    def get_face_candidates(self):
        """
        Runs neural network and returns results for ball class. (Cached)
        """
        self.predict()
        return self._face_candidates

    def draw_face_candidates(self):
        """
        Draw face candidates that had been detected.
        """
        if self._face_candidates is not None:
            for c in self._face_candidates:
                cv2.rectangle(self._image,c.get_upper_left_point(),c.get_lower_right_point(),(0,0,255),3)
        return self._image
...
class YoloHandlerDarknet(YoloHandler):
    ...
    def __init__(self, config, model_path):
        ...
        """
        # Define more paths
        weightpath = os.path.join(model_path, "yolov4-tiny-obj_best.weights")
        configpath = os.path.join(model_path, "yolov4-tiny-obj.cfg")
        datapath = os.path.join("/data/obj.data")
...
class YoloHandlerNCS2(YoloHandler):
    ...
    def __init__(self, config, model_path):
        ...
        self._exec_net = ie.load_network(network=self._net, num_requests=2, device_name=device) #is the num_requests needs correct?
...
class YoloFaceDetector(CandidateFinder):
    """
    A face detector using the yolo neural network.
    This layer connects a single YOLO network with multiple candidate finders for the different classes,
    in this case the face class.
    """
    def __init__(self, config, yolo):
        """
        Constructor for the YoloFaceDetector.
    ...
    def get_candidates(self):
        ...
        """
        return self._yolo.get_face_candidates()
    ...
    def draw_candidates(self):
        ...
        return self._yolo.draw_face_candidates()
